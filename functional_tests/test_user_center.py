...
class UserCenterTest(FunctionalTest):
    ...
    def test_offers_dashboard(self):
        ...
        # 里面有两栏：被采纳的报价和正在报价中
        self.assertEqual(self.browser.find_element_by_css_selector(".adopted_offers h3").text, "被采纳报价")
        self.assertEqual(self.browser.find_element_by_css_selector(".ongoing_offers h3").text, "正在报价中")

        # 其中给山姆的报价在被采纳的报价这栏里
        adopted_offers = self.browser.find_element_by_css_selector(".adopted_offers table")
        ...
        ongoing_offers = self.browser.find_element_by_css_selector(".ongoing_offers table")
        ...
        row.find_element_by_css_selector(".order-initiator").click()
        self.assertRegex(self.browser.current_url, "/deals/supply_offers/details")

        # 左边页面显示采购商品的信息
        self.assertEqual(self.browser.find_element_by_css_selector(".product-container .product-name").text, "USB 64GB")

        # 中间是采购商的具体信息以及他们的采购数量
        self.assertEqual(
            self.browser.find_element_by_css_selector(
                ".purchaser-order-container table tbody tr:first-row .purchaser-name").text, "乔治采购商")
        self.assertEqual(self.browser.find_element_by_css_selector(".purchaser-order-container .amount").text,
                         "100000")
        ...
        row.find_element_by_css_selector(".order-initiator").click()
        self.assertRegex(self.browser.current_url, "/deals/supply_offers/details")
        purchase_order_table = self.browser.find_element_by_css_selector(".purchaser-order-container table")
        ...
        usb_64_order.adopt_supply_offer(supplier=wason)

        # 华少刷新了页面，刚刚乔治的订单从正在报价中转移到了被采纳的报价中
        self.browser.refresh()
        ongoing_offers = self.browser.find_element_by_css_selector(".ongoing_offers table")
        self.assertEqual(len(ongoing_offers.find_element_by_css_selector("tbody tr")), 0)
        adopted_offers = self.browser.find_element_by_css_selector(".adopted_offers table")
        self.assertEqual(len(adopted_offers.find_element_by_css_selector("tbody tr")), 2)

        # 华少点进乔治的订单，页面跳转到了确认订单页面
        adopted_offers = self.browser.find_element_by_css_selector(".adopted_offers table")
        row = adopted_offers.find_element_by_css_selector("tbody tr:nth-child(1)")
        row.find_element_by_css_selector(".order-initiator").click()
        self.assertRegex("/deals/supply_offers/adopt")

        # 华少点击确认订单，页面跳转到了生产订单管理页面
        self.browser.find_element_by_id("id_submit")
