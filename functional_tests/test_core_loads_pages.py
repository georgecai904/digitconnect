...
class CoreLoadsPagesTest(FunctionalTest):
    ...
    def test_user_view_index_page(self):
        ...
        x = self.browser.find_element_by_tag_name("h1")
        print(x.get_property("css"))
        self.assertEqual(self.browser.title, "Direct Connect")

    # def test_nav_bar_title(self):
        # 乔治看到了一条目录栏, 目录栏的名称叫做 “欢迎来到 Direct Connect”
        nav_title = self.browser.find_element_by_css_selector("nav h3")
    ...
    # def test_index_page_index_is_active(self):
        # 其中主页的状态是激活的
        nav_bar_item = self.browser.find_elements_by_css_selector(
            "nav li:first-child")
        self.assertIn("active", nav_bar_item.get_property("css"))

    # def test_index_page_adv_bar(self):
        # 乔治在目录栏的下方看到了新品广告栏
        adv_bar = self.browser.find_element_by_id("adv_bar")

        # 广告栏里面放这一张"欢迎光临"照片
        adv_bar_image = adv_bar.find_element_by_tag_name("img")
        self.assertEqual(adv_bar_image.get_property("src"), "/core/static/images/welcome.jpg")

    # def test_index_page_product_list(self):
        # 乔治在新品广告栏的下方看到买家发布的商品列表
        product_list = self.browser.find_element_by_css_selector(".product_container")

        # 每排显示六个商品
        # [product.get_property("css") for product in product_list]
        if product_list:
            first_product = product_list[0]
            self.assertIn("col-md-2", first_product.get_property["css"])
    ...
    # def test_index_page_product_info(self):
        ...
        if product_list:
            first_product = product_list[0]
            # 每个商品格内显示了产品图片
            self.assertNotEqual(first_product.find_element_by_css_selector(".product_image"), None)
            # 图片下方显示商品信息，包括产品类别，数量，送货地点
            self.assertNotEqual(first_product.find_element_by_css_selector(".product_category"), None)
            self.assertNotEqual(first_product.find_element_by_css_selector(".product_amount"), None)
            self.assertNotEqual(first_product.find_element_by_css_selector(".product_location"), None)
